...
const data = [
  { name: 'Согласовано', value: 520, color: '#10b981' },
  { name: 'На рассмотрении', value: 180, color: '#f59e0b' },
  { name: 'Отклонено', value: 150, color: '#ef4444' },
];

const ApplicationsByApprovalStatusChart = () => {
  return (
    <div className="w-full h-[400px] bg-white dark:bg-slate-800 p-6 rounded-[12px] border border-gray-200 dark:border-slate-700 mt-6">
      <h3 className="text-[18px] font-semibold text-gray-900 dark:text-white mb-6">
        Заявки по статусам согласования
      </h3>
            ...
            <Legend
              ...
              formatter={(value, entry: any) => (
                <span className="text-[14px] font-medium dark:text-gray-300" style={{ color: entry.color }}>
                  {value}: {data.find((d) => d.name === value)?.value} ед.
                </span>
              )}
            />
          </PieChart>
...
export default ApplicationsByApprovalStatusChart;
