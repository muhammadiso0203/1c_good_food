...
const data = [
  { name: 'Тендер (Конкурс)', value: 1.8, color: '#3b82f6' },
  { name: 'Прямой договор', value: 0.96, color: '#f59e0b' },
  { name: 'Товарная Биржа', value: 0.44, color: '#10b981' },
];

const PurchasesByMethodChart = () => {
  return (
    <div className="w-full h-[400px] bg-white dark:bg-slate-800 p-6 rounded-[12px] border border-gray-200 dark:border-slate-700 mt-6">
      <h3 className="text-[18px] font-semibold text-gray-900 dark:text-white mb-6">
        Закупки по способам проведения
      </h3>
            ...
            <Legend
              ...
              formatter={(value, entry: any) => (
                <span className="text-[13px] font-medium dark:text-gray-300" style={{ color: entry.color }}>
                  {value}: {data.find((d) => d.name === value)?.value} млрд сум
                </span>
              )}
            />
          </PieChart>
...
export default PurchasesByMethodChart;
